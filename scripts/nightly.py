...
def create_gcloud_cluster(zone):
  ...
  run(cmd.split())
  time.sleep(300)
  cmd = "kubectl create clusterrolebinding cluster-admin-binding --clusterrole=cluster-admin --user=nightly.cpsgai@gmail.com"
  run(cmd.split()) 
...
if __name__ == "__main__":
  ...
  if not final_result:
    ...
    logging.error("One or more test steps failed exiting with non-zero exit "
                  "code.")
  #create_gcloud_cluster(args.zone)
  get_cluster_info()
  logging.info(args)
